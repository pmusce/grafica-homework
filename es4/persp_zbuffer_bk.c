...
#include "../triangle_rast/triangle_rast.h"
...
void define_object()
{
	...
	x[1]=10.0; y[1]=-10.0; z[1]=-10.0;
	x[2]=10.0;y[2]=10.0;z[2]=-10.0;
	x[3]=-10.0;y[3]=10.0;z[3]=-10.0;
	x[4]=-10.0;y[4]=-10.0;z[4]=10.0;
	x[5]=10.0;y[5]=-10.0;z[5]=10.0;
	x[6]=10.0;y[6]=10.0;z[6]=10.0;
	x[7]=-10.0;y[7]=10.0;z[7]=10.0;
		...
		for (ix=xmin; ix<=xmax; ix++)
			for (iy=ymin; iy<=ymax; iy++)
			{
				...
				if (alpha>=0 && beta>=0 && gamma>=0)
				{
					...
					if(ix >= 0 && ix < xvmax && iy >= 0 && iy < yvmax) {
						if(fz > zbuffer[ix][iy]) {
							zbuffer[ix][iy] = fz;
...
void buffers_create(int w, int h) {
	zbuffer = (float **)malloc(w * sizeof(float *));
    for (i=0; i<w; i++) {
         zbuffer[i] = (float *)malloc(h * sizeof(int));
	}
}

void redraw_scene(SDL_Renderer *ren, SDL_Rect *sub_v, SDL_Surface *tux) {
	SDL_SetRenderDrawColor(ren,255,255,255,255);
	SDL_RenderFillRect(ren, sub_v);
	buffers_init(sub_v->w, sub_v->h);
	draw_mesh(ren,tux);
}
...
int main()
{
	...
	buffers_create(sub_v.w, sub_v.h);
	buffers_init(sub_v.w, sub_v.h);
